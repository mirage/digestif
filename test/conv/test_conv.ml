...
let test_consistent_hex_success i hex =
  Alcotest.test_case (strf "consistent hex:%d" i) `Quick
  @@ fun () -> ignore @@ Digestif.SHA1.consistent_of_hex hex

let test_hex_success i hex =
  Alcotest.test_case (strf "hex:%d" i) `Quick
  @@ fun () -> ignore @@ Digestif.SHA1.of_hex hex

let test_consistent_hex_fail i hex =
  Alcotest.test_case (strf "consistent hex fail:%d" i) `Quick
  @@ fun () ->
  try
...
let tests () =
  Alcotest.run "digestif"
    ...
    ; "consistent_of_hex 0", List.mapi test_consistent_hex_success consistent_hex
    ...
    ; "consistent_of_hex 2", List.mapi test_consistent_hex_fail inconsistent_hex
    ; ( "iso of_hex"
