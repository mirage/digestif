...
let hex str =
  let hexdigit = function
    ...
    | x               -> raise (Invalid_argument (sp "of_hex: `%c'" x))
  in
  ...
  match
    ...
    ~f:(fun (cs, i, acc) -> function
        ...
        | char ->
            ...
            | (Some y, x) -> Bi.set cs i (Char.unsafe_chr (x lor y)) ; (cs, succ i, None))
    ~z:(Bi.create (String.length str), 0, None)
    str
  with
  | (_ , _, Some _) -> raise (Invalid_argument "of_hex: dangling nibble")
  | (cs, i, _     ) -> Bi.sub cs 0 i
...
let md5_inputs =
  ...
  let keys =
    List.mapi (fun i x ->
      if i == 3 || i == 5 || i == 6 then
        x
      else Bi.(sub x 0 (min (length x) 16)))
    k in
  List.combine keys d
...
let blake2b_inputs =
  let k, d = List.split sha1_inputs in
  let keys =
    List.mapi (fun i x -> Bi.(sub x 0 (min (length x) 64)))
    k in
  List.combine keys d
...
let assert_bigstring_equal a b =
  if Bi.equal a b
  then ()
  else assert false

let bigstring : Bi.t Alcotest.testable = let module M = TestBigstring in (module M)

let test hash i ((key, data), result) =
  let computed = Rakia.Bigstring.mac hash ~key:key data in
  Alcotest.(check bigstring) "hmac" result (if i == 4 then Bi.(sub computed 0 (length result)) else computed)

let test_hmac name id =
  List.mapi (fun i args ->
    let title = "HMAC " ^ name ^ " " ^ string_of_int i in
    title,
    `Slow,
    (fun () -> test id i args))
...
let () =
  Alcotest.run "Rakia"
    [ "MD5", test_hmac "MD5" `MD5 (List.combine md5_inputs md5_results)
    ; "SHA1", test_hmac "SHA1" `SHA1 (List.combine sha1_inputs sha1_results)
    ; "SHA224", test_hmac "SHA224" `SHA224 (List.combine sha2_inputs sha224_results)
    ; "SHA256", test_hmac "SHA256" `SHA256 (List.combine sha2_inputs sha256_results)
    ; "SHA384", test_hmac "SHA384" `SHA384 (List.combine sha2_inputs sha384_results)
    ; "SHA512", test_hmac "SHA512" `SHA512 (List.combine sha2_inputs sha512_results)
    ; "BLAKE2B", test_hmac "BLAKE2B" `BLAKE2B (List.combine blake2b_inputs blake2b_results) ]
