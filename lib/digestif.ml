  ...
  match
    ...
    ~f:(fun (cs, i, acc) -> function
        ...
        | char ->
            ...
            | (Some y, x) -> Bigstring.set cs i (Char.unsafe_chr (x lor y)) ; (cs, succ i, None))
    ~z:(Bigstring.create (String.length str), 0, None)
    str
  with
  | (_ , _, Some _) -> raise (Invalid_argument "of_hex: dangling nibble")
  | (cs, i, _     ) -> Bigstring.sub cs 0 i
...
let md5_inputs =
  ...
  let keys =
    List.mapi (fun i x ->
      if i == 3 || i == 5 || i == 6 then
        x
      else Bigstring.(sub x 0 (min (length x) 16)))
    k in
  List.combine keys d
...
let blake2b_inputs =
  let k, d = List.split sha1_inputs in
  let keys =
    List.mapi (fun i x -> Bigstring.(sub x 0 (min (length x) 64)))
    k in
  List.combine keys d
...
let assert_bytes_equal a b =
  if Bigstring.equal a b
  then ()
  else begin
    Format.printf "a:%a = b:%a\n%!"
      pp (Bigstring.to_string a) pp (Bigstring.to_string b);
    assert false
  end
...
let test hash i ((key, data), result) =
  ...
  if i == 4 (* truncated thingy *) then
    assert_bytes_equal result Bigstring.(sub computed 0 (length result))
  else
    assert_bytes_equal result computed
...
let hmac_suite =
  ...
  test_hmac "BLAKE2B" `BLAKE2B (List.combine blake2b_inputs blake2b_results)

let () =
  List.iter (fun x -> x ()) hmac_suite
