...
struct
  ...
  struct
    external init     : ctx -> unit
                      = "caml_digestif_ripemd160_ba_init"
                      [@@noalloc]
    external update   : ctx -> ba -> off -> size -> unit
                      = "caml_digestif_ripemd160_ba_update"
                      [@@noalloc]
    external finalize : ctx -> ba -> off -> unit
                      = "caml_digestif_ripemd160_ba_finalize"
                      [@@noalloc]
  end
  ...
  struct
    external init     : ctx -> unit
                      = "caml_digestif_ripemd160_st_init"
                      [@@noalloc]
    external update   : ctx -> st -> off -> size -> unit
                      = "caml_digestif_ripemd160_st_update"
                      [@@noalloc]
    external finalize : ctx -> st -> off -> unit
                      = "caml_digestif_ripemd160_st_finalize"
                      [@@noalloc]
  end

  external ctx_size   : unit -> int
                      = "caml_digestif_ripemd160_ctx_size"
                      [@@noalloc]
end
