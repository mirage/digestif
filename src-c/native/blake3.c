/* BLAKE3 C implementation, version 1.8.7.
 * Upstream: https://github.com/BLAKE3-team/BLAKE3/tree/f3149ec5bb5449af877ba20377a11008ff499fa2/c
 * Upstream sections are CC0 1.0; see blake3.LICENSE.
 *
 * The marked upstream sections combine blake3_impl.h, blake3_portable.c,
 * and blake3.c. Local header includes and the unused load_block_words helper
 * are omitted; trailing whitespace is normalized. Digestif sections provide
 * symbol prefixes, portable compression, scratch-buffer sizing, and the XOF loop.
 * OCaml bindings are in stubs.c; the pure OCaml backend is separate.
 */
#include "blake3.h"

/* BEGIN Digestif: portable symbol mapping */
#define blake3_compress_in_place digestif_blake3_compress_in_place
#define blake3_compress_in_place_portable digestif_blake3_compress_in_place
#define blake3_compress_xof digestif_blake3_compress_xof
#define blake3_compress_xof_portable digestif_blake3_compress_xof
#define blake3_hash_many digestif_blake3_hash_many
#define blake3_hash_many_portable digestif_blake3_hash_many
#define blake3_xof_many digestif_blake3_xof_many
#define blake3_simd_degree digestif_blake3_simd_degree
#define blake3_compress_subtree_wide digestif_blake3_compress_subtree_wide
/* END Digestif: portable symbol mapping */

/* BEGIN upstream: c/blake3_impl.h */
...
#endif /* BLAKE3_IMPL_H */
/* END upstream: c/blake3_impl.h */

/* BEGIN Digestif: portable scratch-buffer sizing */
#undef MAX_SIMD_DEGREE
#define MAX_SIMD_DEGREE 1
/* END Digestif: portable scratch-buffer sizing */

/* BEGIN upstream: c/blake3_portable.c */
#include <string.h>
...
/* END upstream: c/blake3_portable.c */

/* BEGIN Digestif: portable dispatch */
size_t digestif_blake3_simd_degree(void) { return 1; }

/* The portable equivalent of upstream's dispatched full-block XOF helper. */
void digestif_blake3_xof_many(const uint32_t cv[8],
                             const uint8_t block[BLAKE3_BLOCK_LEN],
                             uint8_t block_len, uint64_t counter, uint8_t flags,
                             uint8_t out[64], size_t outblocks) {
  for (size_t i = 0; i < outblocks; i++) {
    digestif_blake3_compress_xof(cv, block, block_len, counter + i, flags,
                                out + i * 64);
  }
}
/* END Digestif: portable dispatch */

/* BEGIN upstream: c/blake3.c */
#include <assert.h>
#include <stdbool.h>
#include <string.h>
#include <stdint.h>
